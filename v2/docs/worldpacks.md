...
### Avatar identity and levels

Actor rows may add an `identity` policy and `level_track_id`. An identity separates the
canonical description from `persona` and observable `appearance`, declares `authored`,
`hybrid`, or `self_authored` authority, and lists the traits that may evolve. Authored is
the backward-compatible default. Self-authored and hybrid actors must have their own exact
two-way text model binding; their per-level self-description may not be generated by a
different model.

The optional `avatar_level_tracks.json` resource defines reusable progression for explicit
`actor_ids` or for a pack-and-speech-mode selector. Levels are contiguous from 1 through
`max_level` (currently bounded at 20) and may require counts of successful committed event
types, distinct locations, a kernel-owned ability check, bounded hit-point growth, and
authored appearance permissions. A failed chance gate records the attempt and becomes
eligible again only when new qualifying evidence exists.

Level evidence is replay-safe world causality. Chat messages, model activity, generated
prose, thoughts, dreams, self-descriptions, Orb balances, and other currency events are
rejected as advancement requirements. The legacy item-placement evolution resource remains
readable for existing packs, while new packs should use avatar level tracks when they need
more than the old level-2 transformation.
...
attempt `Embodied` and `World-Known` only after committed exploration and custody evidence.
Wisdom and Charisma gates supply genuine chances to level; further movement creates a new
