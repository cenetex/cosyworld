...
use raw speech. A text-input, image-output avatar keeps
`speech_mode: "unavailable"`—it cannot speak—but can answer a grounded room
heartbeat with one moderated `image.created` reply generated by its exact
checked-in model binding. Other unsupported output modalities remain visible
