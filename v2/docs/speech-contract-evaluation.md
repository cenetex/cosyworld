...
The deterministic gate checks particular speech properties. It is not a general
proof of every factual claim. The comparison therefore includes a manual read
...
responses. It makes no network requests. Its inputs are
`COSYWORLD_SPEECH_EVAL_DIR` and `COSYWORLD_SPEECH_EVAL_MODEL`.

## Result

Provider sampling is pending. The production prompt remains unchanged until
the measured comparison is complete.
