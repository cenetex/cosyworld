...
After `Your first tale` finishes, the guide hands off to one grounded **room thread** drawn from authoritative world state: a wanted gift, urgent care, shared work, danger, an open path, something still hidden, a nearby voice, or finally the room's authored hook. Threads use story language rather than quest counters, and actionable threads can be selected to bring their matching scene card into the hand.
...
When no AI key is configured, avatar chat and resident replies use deterministic local text. In both modes pressing `Chat` creates a short four-beat exchange—avatar, resident, avatar, resident—for one Orb. With AI configured, the selected model authors the opening avatar line; the continuation engine supplies the remaining contract-safe beats immediately, so a slow provider cannot strand half a conversation. Every line is committed through the C kernel as `CW_ACTION_SAY`, validated against its speaker's speech contract, and broadcast as a shared world event. If a generated beat repeats a recent line, the runtime rotates to a contract-safe alternate instead of silently dropping the promised reply; emoji and emote residents keep their authored speech modes. The exchange leaves one friendly witness memory per resident instead of farming a new reward on every repeat. Human-authored room speech is a separate moderated `say` path.
...
CosyWorld time is player-powered. There is no real-time ambient scheduler: the world does not commit resident speech, checks, danger ticks, or placement changes simply because wall-clock seconds passed. Each confirmed scene card calls on the next active resident in room-card order to react; a direct target reply takes precedence so Give, Trade, and friendship cards do not answer twice. The deterministic continuation recognizes arrival, discovery, keepsakes, growth, purpose, practice, friendship, work, rest, danger, and preparation, then answers in the resident's prose, emoji, emote, or multi-voice contract instead of dropping an unrelated stock line. That reaction is committed and broadcast before the card request resolves, keeping its conversational beat ahead of the next card or Orb Chat. The room transcript keeps one story-language card outcome immediately before the answer—what the avatar heard, found, carried, changed, or helped—while projection tags, ledgers, and advancement bookkeeping stay out of the scene. Player actions can also fan out into lifecycle hooks, frontier danger/progress clocks, and player-turn encounter resets, all through the normal audited journal path. `COSYWORLD_AMBIENT_QUIET_SECS` remains only as a local test threshold for ambient helper coverage; `/meta.features.ambient_enabled` reports `false` in the running service.
...
- The server authors an in-character avatar opener, the resident replies, the avatar follows up, and the resident closes; deterministic fallback text keeps the same cadence when no model is configured.
