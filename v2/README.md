...
On startup, the orchestrator replays `action_journal` when it is present. JSON snapshots are an accelerator and fallback, not the source of truth. The startup log line reports how many milliseconds boot took from process start to listening, so a regression to full replay is visible. A rejected checkpoint is also surfaced beyond logs: `/meta.persistence` reports `checkpoint_rejections` and `last_checkpoint_rejection` for the running process. Checkpoint validation distinguishes unbound state from divergence: funded-but-unbegun generated media carries an empty policy binding and adopts its pathway's binding on load, exactly as begin would, while a non-empty binding that disagrees with its pathway still fails closed.
