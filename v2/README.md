...
At the final first-tale choice, both `Chat` and `Expand Bracelet` stay visibly guided in the hand. The player can therefore make the promised friendship-or-charm-slot choice directly, including on the two-card mobile hand, instead of relying on a redraw. `Expand Bracelet` names the next slot and the one banked growth it spends; it never implies an XP level-up or grants the skill charm itself. A successful Listen reveals one vivid lead at a time. The collapsed room log keeps the latest human card outcome on top while derived memory rewards and resident ripples remain in the expanded chronological history.
