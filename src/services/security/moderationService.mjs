...
Respond ONLY with one of: low, medium, high, and a brief reason explaining the risk.`;
      ...
      if (threatLevel === 'high') {
        await this.riskManagerService.storeHighRiskMessage({
          messageId: message.id,
          channelId: message.channel.id,
          guildId: message.guild?.id,
          content,
        ...
        const tags = await this.aiService.generateTagsForContent(content);
        await this.riskManagerService.updateMessageTags(message.id, tags);
      }
