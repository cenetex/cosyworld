...
export class PlanExecutionService {
   ...
   */
  constructor({ logger, executorRegistry = null } = {}) {
    this.logger = logger;
  ...
  async executePlan(plan, context, options = {}) {
    ...
    this.logger?.info?.(`[PlanExecutionService] Starting plan execution with ${totalSteps} steps`);
    ...
    for (let i = 0; i < plan.steps.length; i++) {
      ...
      // Notify progress
      if (onProgress) {
        await onProgress(stepNum, totalSteps, action);
      }

      this.logger?.info?.(`[PlanExecutionService] 📍 Step ${stepNum}/${totalSteps}: ${(action || 'unknown').toUpperCase()}`);
