...
export class ConversationManager  {
  ...
  async _generateImageOnlyResponse(avatar, channel, channelHistory = []) {
    try {
      ...
      const imageUrl = result.url || (result.data ? `data:image/png;base64,${result.data}` : null);
      
      if (!imageUrl) {
        this.logger?.warn?.(`[ConversationManager] No image URL from generation for ${avatar.name}`);
        return null;
      }
      ...
      // Send via webhooks if available
      const message = await this.discordService.sendMessage({
