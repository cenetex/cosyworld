...
export class LocationService  {
  ...
  this.unifiedAIService = unifiedAIService; // optional adapter
    ...
    // Late binding to avoid cyclic dependency
 ...
 */
  async getLocationByChannelId(channelId) {
    this.db = await this.ensureDbConnection();
 ...
 */
   ...
   */
  async generateLocationSummary(locationId, channelHistory) {
    try {
      const { location, avatars } = await this.getMapService().getLocationAndAvatars(locationId);
      if (!location) {
