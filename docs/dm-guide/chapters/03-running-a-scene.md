...
::: {.target}
**ACCEPTED TARGET - Consequence-first discovery.** Safe exact methods already
work without arbitrary rolls. The rule that pressured Search and Focused
Notice always resolve fixed truth while a check determines only the named
consequence belongs to the unified discovery rollout.
:::
...
Read-only examination is free. Under the Accepted Target discovery procedure,
Focused Notice, Search, Study, forcing, disarming, opening, taking, and
crossing use their declared costs. If a Sign is needed to understand danger,
show it for free. If the player wants hidden mechanism, exact provenance,
exhaustive certainty, or a safer method, offer a focused action.
