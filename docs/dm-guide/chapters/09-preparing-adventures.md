...
::: {.target}
**ACCEPTED TARGET - Forward-compatible preparation.** Passes involving
Discovery Slots, typed obstacles, Anchors, cairns, and Scout v2 describe
accepted authoring targets. They do not make those interactions available in
the current runtime.
:::
...
This authoring example combines Core behavior, Accepted Target procedures for
shared discovery, typed obstacles, Scout v2, and cairns, and the optional
short-rest Playtest.
...
::: {.target}
**ACCEPTED TARGET - Scout v2 use.** The party begins a foray from the Gate and
follows one active Lead through provisional ground. The approaching rain may
weaken the trail. Success reveals the Washout and its edge; failure also
reveals it while advancing rain and reducing legibility. Neither result moves
the party. There is no unchanged reroll.
:::

Under shipped Scout, this scene instead reveals the prepared adjacent path
edge without movement. The party then Travels to the Washout, recording the
traversed return leg.
