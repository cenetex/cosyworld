...
- The card-hand control surface: two dealt action cards with art and labels, a complete grouped action chooser, a detail/confirm surface, and an optional free deterministic redeal — play feels like turns, not clicking.
...
5. **One meaningful action hand.** The resting UI is two labeled suggestions plus a compact **all actions** affordance, all derived from the same server-authored legal offer set. The chooser groups that complete set by intention and exposes every target; it is a rendering, not parallel authority. An optional free redeal changes only the two suggestions. Each selected action shows its target, cost, and risk before commit. Playing an action is a turn; redealing and speech (`say`, `/me`) are turn-exempt. The small hand is a focus mechanism, not an inventory limit.
...
**Rule 2 — the action hand is the controller.** The shipped resting surface is a dealt hand of two labeled action cards plus a compact chooser that renders the complete current legal offer set by intention and target. Selecting either a suggestion or chooser row opens the same detail surface naming target, cost, and risk before commit. Optional redeal replaces only the visible pair with the next pair from the finite authoritative order; it is free browsing, not a random draw, and cannot change legality or silently re-rank the opening hand. The same contract projects onto every future transport: cards become Discord reactions, terminal keys, or voice intents without new server concepts. Three laws hold regardless of transport: every action carries a visible label, the complete server-authored legal set is directly reachable, and alternate browsing never creates authority.
...
- The resting UI is one action controller: two labeled suggestions plus a compact complete-offer chooser, server-derived; no permanent composer, send button, or navigation sidebar. Optional redealing is free; playing spends a turn.
...
- **Scene composition can become illegible.** The underlying merge may be deep while the resting surface stays small. Keep the two-card action hand, make precedence inspectable, and test that the grouped chooser preserves every current legal action and target even when neither is initially suggested.
